...
우리가 원하는 것은 그런 것이 아닐 것입니다. 우리는 divider가 카드의 전체 너비에 걸쳐 있고 각 섹션이 동일하게 패딩되기를 원합니다. 약간 hacky한 CSS를 사용해 볼 수도 있습니다. 만약 제가 이 컴포넌트가 있는 코드베이스에서 작업했다면 다음과 같은 것을 보고도 놀라지 않았을 것입니다:
...
이 방법은 동작은 하겠지만, 가장 강력한 해결책은 아닙니다. 다시 앞으로 나아가기 위해서는 hacky CSS를 작성하는 대신 한 발짝 물러서야 합니다. 컴포지션은 이를 위해 도움이 될 것입니다.
...
바로 여기서 `Card`에 과부하가 걸린 것을 인식하고 이를 분리하는 작업이 바로 컴포넌트 기반 디자인 시스템을 구축하는 작업입니다. 이것이 바로 이러한 구성 요소를 견고하고 유연하게 만드는 방법입니다.
...
이 문제가 단순한 `Card` 컴포넌트에만 영향을 미치는 것이 아니라, 스타일링 책임이 너무 많은 다른 위법 사항이 있다는 것을 여러분의 상상력이 도움이 되길 바랍니다. 가장 흔한 것 중 하나는 [exporting margins](https://mxstbr.com/thoughts/margin/)이지만 이에 대한 글은 다음 기회로 미루겠습니다.

일부 UI가 원하는 방식으로 작동하는 데 어려움을 겪고 있다면, 한 발 물러서서 한 걸음 더 나아갈 수 있도록 노력하세요. 컴포넌트의 일부에 과부하가 걸리는지, 해당 책임을 분리해도 문제가 해결되지 않는지 살펴보세요.